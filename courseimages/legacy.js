/* CourseLab ToolBox
 v.2.7 20160216
(c)2002-2016 Websoft Ltd. http://www.courselab.ru/ http://www.courselab.com/ */
...
function ConvertPeriodToISO8601(iTime)
{
	...
	var iSec = Math.floor(iTime/1000);
	var iMS2 = Math.floor(iTime - iSec);
	var iDays = 0;
	var iHours = 0;
	var iMin = 0;
	var iSec = 0;
	if(iSec > 86399)
	{
		iDays = Math.floor(iSec/86400);
		iSec = iSec - 86400*iDays;
	}
	if(iSec > 3599)
	{
		iHours = Math.floor(iSec/3600);
		iSec = iSec - 3600*iHours;
	}
	if(iSec > 59)
	{
		iMin = Math.floor(iSec/60);		iSec = iSec - 60*iMin;
	}
	var sMS2 = iMS2.toString();
	if(sMS2.length==1)
	{
		sMS2 = "0" + sMS2;
	}
	...
	if(iDays != 0)
	{
		sValue += iDays.toString()+"D";
	}
	sValue += "T" + iHours.toString() + "H" + iMin.toString() + "M" + iSec.toString() + "S." + sMS2;
	return sValue;
}
function ConvertDateToISO8601(dDate)
{
	var iMonth = dDate.getUTCMonth() + 1;
	var sMonth = iMonth.toString();
	if(sMonth.length==1)
	{
		sMonth = "0" + sMonth;
	}
	var iDate = dDate.getUTCDate();
	var sDate = iDate.toString();
	if(sDate.length==1)
	{
		sDate = "0" + sDate;
	}
	var iHours = dDate.getUTCHours();
	var sH = iHours.toString();
	if(sH.length==1)
	{
		sH = "0" + sH;
	}
	var iMin = dDate.getUTCMinutes();
	var sM = iMin.toString();
	if(sM.length==1)
	{
		sM = "0" + sM;
	}
	var iSec = dDate.getUTCSeconds();
	var sS = iSec.toString();
	var iMS = dDate.getUTCMilliseconds();
	var iTmp = Math.floor(iMS/10);
...
function QTIEvalVarConditionBlock(oConditionRoot, aResponse, sQType)
{
	...
	for(var i=0; i<aConditions.length; i++)
	{
		...
		switch(aConditions[i].nodeName)
		{
			...
			default:
			{
				if(sEvalString!="") sEvalString += (sQType=="string") ? " || " : " && ";
	...
	switch(sQType)
	{
		...
		case "order":
		{
			...
			for(var i=0; i<aConds.length; i++)
			{
				...
				{
					return (aResponse[iPos]==sCondText);
				}
			}
			break;
		}
