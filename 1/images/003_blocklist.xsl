...
<xsl:template match="params">
	...
	<xsl:variable name="cur_folder"><xsl:value-of select="$imagesFolder"/><xsl:value-of select="listblock_profile"/>/</xsl:variable>
	...
	<xsl:for-each select="table/item">
		...
		<table width="100%"  border="0" cellpadding="0" cellspacing="0">
			...
			<xsl:if test="../../eff='yes'">
				<xsl:attribute name="style">display: none; visibility:hidden; margin-bottom:<xsl:value-of select="../../items_margin"/>px; filter:revealtrans(duration=<xsl:value-of select="../../durat"/>, transition=<xsl:value-of select="../../trans"/>)</xsl:attribute>
			</xsl:if>
			<xsl:if test="../../eff='no'">
				<xsl:attribute name="style">display: none; visibility:hidden; margin-bottom:<xsl:value-of select="../../items_margin"/>px;</xsl:attribute>
			</xsl:if>
			<tr>
				<td>
	...
	<xsl:if test="on_link='yes'">
		<div>
			...
			<a href="#">
				<xsl:attribute name="onclick">var oArgs=new Object; oArgs.pid='<xsl:value-of select="$objectID"/>'; CallMethod('003_blocklist', 'ShowItem',oArgs); return false;</xsl:attribute>
