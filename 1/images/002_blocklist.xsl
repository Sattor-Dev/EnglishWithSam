...
<xsl:for-each select="list/item">
	<table width="100%" cellpadding="0" cellspacing="0">
		...
		<xsl:if test="../../eff='yes'">
			<xsl:attribute name="style">display: none; visibility: hidden; <xsl:if test="position()!=1">margin-top: <xsl:value-of select="../../items_margin"/>px;</xsl:if> filter:revealtrans(duration=<xsl:value-of select="../../durat"/>, transition=<xsl:value-of select="../../trans"/>)</xsl:attribute>
		</xsl:if>
		<xsl:if test="../../eff='no'">
			<xsl:attribute name="style">display: none; visibility: hidden; <xsl:if test="position()!=1">margin-top: <xsl:value-of select="../../items_margin"/>px;</xsl:if></xsl:attribute>
		</xsl:if>
		<tr>
			<td width="100%">
...
<xsl:if test="mode='click'">
	<xsl:if test="click_mode='link'">
		<div>
			...
			<a href="#">
				<xsl:attribute name="onclick">var oArgs=new Object; oArgs.pid='<xsl:value-of select="$objectID"/>'; CallMethod('002_blocklist', 'NextItem',oArgs); return false;</xsl:attribute>
